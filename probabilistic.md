---
layout: default
---

## Probabilistic Modeling

### Bayes-by-backprop
*  <https://www.nitarshan.com/bayes-by-backprop/>
*  <https://medium.com/neuralspace/probabilistic-deep-learning-bayes-by-backprop-c4a3de0d9743>

### Probabilistic Modeling Packages

*  [tensorflow probability](https://github.com/tensorflow/probability/blob/master/tensorflow_probability/examples/bayesian_neural_network.py "BNN example")
*  [PyMC3](https://gist.github.com/anonymous/96b998304de1eb4306738543170788ca "BNN example")
*  [Edward](https://www.ibm.com/blogs/research/2018/11/edward-wml/ "BNN example")
    - [Edward batch training](http://edwardlib.org/tutorials/batch-training)
*  [Pyro](https://github.com/paraschopra/bayesian-neural-network-mnist/blob/master/bnn.ipynb "BNN example")
